-- Utilized variations of this to get ultimate lists compiled. Used Excel/Python to segment, filter, etc. -- 
SELECT DISTINCT user_id, equipment, personal__firstname, personal__lastname, login__ifit__email, shipping__country, series_complete
FROM (
SELECT *
FROM (
...
SELECT *,
    ...
    CASE WHEN series = 'Bike - Level 1' THEN 'A1'
     ...
     CASE WHEN series = 'Bike - Level 1' THEN 'Bike'
            WHEN series = 'Bike - Level 2' THEN 'Bike'
            WHEN series = 'Bike - Level 3' THEN 'Bike'
            WHEN series = 'Tread - Level 1' THEN 'Tread'
            WHEN series = 'Tread - Level 2' THEN 'Tread'
            WHEN series = 'Tread - Level 3' THEN 'Tread'
            WHEN series = 'Elliptical - Level 1' THEN 'Ellip'
            WHEN series = 'Elliptical - Level 2' THEN 'Ellip'
            WHEN series = 'Elliptical - Level 3' THEN 'Ellip'
            WHEN series = 'Rower - Level 1' THEN 'Rower'
            WHEN series = 'Rower - Level 2' THEN 'Rower'
            WHEN series = 'Rower - Level 3' THEN 'Rower'
     END AS equipment
FROM (
    ...
    FROM (
        ...
        AND ul."start"::date BETWEEN '2020-08-05' AND '2020-09-07'
        )
...
SELECT series_users.user_id, u.login__ifit__email, u.personal__firstname, u.personal__lastname, u.shipping__street1,
        ...
        DATEDIFF(year,personal__birthday,getdate()) as user_age, series_users.equipment
FROM series_users
JOIN prodmongo.users u on series_users.user_id = u._id
)
WHERE shipping__country = 'US'
ORDER BY user_id, partition_key DESC
)
WHERE equipment = 'Tread'
ORDER BY user_id, equipment
