...
SELECT membership_type, count(DISTINCT users_id)
from (
/*SELECT *,
       row_number() over (partition by users_id order by date_part(epoch, "date") DESC) as ord_epochs
FROM ( */
SELECT us.*,
       nu.mst_membership_start_date,
       nu.subscription_set_to,
       nu.payment_set_to
from user_set us 
join new_users nu on us.users_id = nu.users_id
        AND CONVERT_TIMEZONE('AMERICA/DENVER',us."date")::DATE = nu.mst_membership_start_date
WHERE mst_membership_start_date >= '2021-01-01' --BETWEEN '2021-01-01' AND '2021-12-31'
AND primary_or_sec = 'primary'
AND employee = 0
AND user_type = 'Paid'
--)
)
